...
export default (test) => {
  ...
  test('POST /api/user/:id - should throw error if new passwords do not match', (t) => {
    request(app)
      .post(`/api/user/${app.get('user').id}`)
      .set('x-access-token', app.get('token'))
      .send({password: '12345', passwordRepeat: '123'})
      .expect(400)
      .expect('Content-Type', /json/)
      .end((err, res) => {
  ...
  test('POST /api/user/:id - update with existing login', (t) => {
    request(app)
      .post(`/api/user/${app.get('user').id}`)
      .set('x-access-token', app.get('token'))
      .send({login: 'notthesame'})
      .expect(400)
      .expect('Content-Type', /json/)
      .end((err, res) => {
        const expectedBody = {error: 'Login is already taken!'};
        const actualBody = res.body;
  ...
  test('POST /api/user/:id - update with new password', (t) => {
    request(app)
      .post(`/api/user/${app.get('user').id}`)
      .set('x-access-token', app.get('token'))
      .send({password: 'qwe', passwordRepeat: 'qwe'})
      .expect(200)
      .expect('Content-Type', /json/)
      .end((err, res) => {
        const expectedBody = {
        ...
        t.error(err, 'No error');
        t.deepEqual(actualBody, expectedBody, 'Retrieve new password');
        t.notOk(actualBody.password, 'No password included');
        t.end();
      });
  });
  ...
  test('Should login with updated username and password', (t) => {
    request(app)
      .post('/api/login')
      .send({login: 'test123', password: 'qwe'})
      .expect(200)
      .expect('Content-Type', /json/)
      .end((err, res) => {
        const actualBody = res.body;
