...
export default (test) => {
  test('Should login with given username and password', (t) => {
    request(app)
      .post('/api/login')
  ...
  test('Should login with another username and password', (t) => {
    request(app)
      .post('/api/login')
      .send({login: 'notthesame', password: '123'})
      .expect(200)
      .expect('Content-Type', /json/)
      .end((err, res) => {
        const actualBody = res.body;
        ...
        t.equal(actualBody.user.login, 'notthesame', 'Login matches request');
        t.notOk(actualBody.user.password, 'No password included');
        t.deepEqual(actualBody.user, decodedUser, 'User must match token');
